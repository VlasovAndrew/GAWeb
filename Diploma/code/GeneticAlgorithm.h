#pragma once

#include <vector>
#include <algorithm>
#include <set>
#include <ctime>

#include "GraphWork.h"
#include "Random.h"
#include "Strassen.h"
#include "VectorOutput.h"

using std::begin;
using std::end;
using std::max_element;
using std::min_element;
using std::pair;
using std::vector;
using std::set;

// Класс для работы с генетическим алгоритмом
class GeneticAlgorithm {
private:
	// Ссылка на граф
	Graph* G;
	// Размер популяции
	int populationSize;
	// Параметры, отвечающие за мутацию и скрещивание
	double mutationP, crossP;
	// Вектор с набором вершин в популяции
	vector<int> population;
	// Модуль декотратор для работы с генерацией псевдо случайных величин
	Random rd;
	// Метод отвечающий за старт алгоритма,
	// принимает ссылку на переменную для записи времени работы алгоритма.
	void startAlgorithm(double& time) {
		// число итераций 
		int stepN = 20;
		// начало измерения времени работы алгоритма
		double start = clock();
		// итерационное выполнение алгоритма
		for (int i = 0; i < stepN; i++) {
			this->evolutionStep();
		}
		// окончание измерения времени
		double finish = clock();
		// вычисление времени работы всего алгоритма
		time = (finish - start) / CLOCKS_PER_SEC;
	}

public:
	// Конструктор класса, принимает ссылку на граф, размер популяции,
	// вероятность скрещивания и вероятность мутации
	GeneticAlgorithm(Graph* g, int popSize, double pC, double pM) {
		// инициализация внутренних полей класса
		this->G = g;
		this->populationSize = popSize;
		this->crossP = pC;
		this->mutationP = pM;
		// Генерация случайной начальной популяции
		for (int i = 0; i < popSize; i++) {
			population.push_back(rand() % g->Size());
		}
	}

	// Этап селекции
	void makeSelection() {
		// Поиск эксцентриситетов всех вершин в популяции
		vector<int> e;
		for (int i = 0; i < population.size(); i++) {
			e.push_back(G->getEccentricity(population[i]));
		}
		// вычисление вероятности для попадания в следующее поколение
		// вычисление ее таким образом, чтобы вероятность была выше 
		// у вершин с меньшим эксцентриситетом
		vector<double> probability;
		double maxV = *max_element(e.begin(), e.end()), leftSum = 0;
		for (int i = 0; i < e.size(); i++) {
			leftSum += maxV / double(e[i]);
		}
		double x = 1.0 / leftSum;
		for (int i = 0; i < e.size(); i++) {
			probability.push_back(maxV / double(e[i]) * x);
		}
		// этап селекции на основе вычисленных вероятностей
		vector<int> nextPopulation;
		for (int i = 0; i < this->populationSize; i++) {
			nextPopulation.push_back(rd.choice(population, probability));
		}
		this->population = nextPopulation;
	}
	// этап скрещивания
	void crossing() {
		// вектор для результата скрещивания
		vector<int> crossed;
		for (int i = 0; i < populationSize; i++) {
			// скрещивание происходит с заданной вероятностью 
			if (rd.getRandomLowerOne() < crossP) {
				// выбор индексов вершин из популяции
				int ind1 = rd.randint(0, populationSize - 1);
				int ind2 = rd.randint(0, populationSize - 1);
				// получение самих вершин
				int u = population[ind1], v = population[ind2];
				// поиск кротчайших путей между вершинами
				vector<int> path = G->getPath(u, v);
				// выбор вершины из середины пути
				crossed.push_back(path[path.size() / 2]);
			}
		}
		// добавление потомков в популяцию
		this->population.insert(population.end(), crossed.begin(), crossed.end());
	}
	// процесс мутации
	void mutation() {
		for (unsigned int i = 0; i < this->population.size(); i++) {
			// с учетом вероятности мутации
			if (rd.getRandomLowerOne() < mutationP) {
				// поиск соседей 
				vector<int> n = G->getNeighbour(population[i]);
				// выбор одного из соседей
				population[i] = rd.choice(n);	
			}
		}
	}
	// вывод популяции
	void printPopulation() {
		for (int i = 0; i < this->population.size(); i++) {
			cout << population[i] << " ";
		}
		cout << endl;
	}
	// метод для эволюционной итерации
	void evolutionStep() {
		// скрещивание
		this->crossing();
		// селекция
		this->makeSelection();
		// мутация
		this->mutation();
	}
	// поиск вершины с минимальным эксцентриситетом после работы алгоритма
	int getBestResult(double& time) {
		// запуск генетического алгоритма
		this->startAlgorithm(time);
		// вектор для хранения эксцентриситетов
		vector<int> e;
		// поиск эксцентриситетов для всех вершин в популяции
		for (int i = 0; i < population.size(); i++) {
			int x = G->getEccentricity(population[i]);
			e.push_back(x);
		}
		// возвращение индексов вершин эксцентриситетов
	 	vector<int> ind = this->getCentralVertex(e);
		// возвращение первой из найденной оптимальной вершины
		return this->G->getEccentricity(population[ind[0]]);
	}
	// получение популяции
	vector<int> getPopulation() {
		return this->population;
	}
	// поиск индексов вершин с минимальным эксцентриситетов
	vector<int> getCentralVertex(vector<int> e) {
		// минимальный эксцентриситет
		int minV = *min_element(e.begin(), e.end()); 
		// выделение индексов минимальных эксцентриситетов
		vector<int> res;
		for (int i = 0; i < e.size(); i++) {
			if (e[i] == minV) {
				res.push_back(i);
			}
		}
		return res;
	}
};
